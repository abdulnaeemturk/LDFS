<x-app-layout>
    ...
    <form method="POST" action="#" id="add_form">
        @include('Project.devaitionractification.partials.form')
...
function addRecord()
{
    var action_completed = submitFormDynamically('add_form', 'devaitionractification', 'Successfully Inserted', 'Please Try Again', 'POST');
...
{
    getFormRecordDynamically("update_form", 'devaitionractification', _record_id, "get", "update_record", "add_form", "update_form");
}

function updateRecord(_record_id)
{
    var action_completed = submitFormDynamically('update_form', 'devaitionractification/'+_record_id, 'Successfully Updated', 'Please Try Again', 'POST');
    if(action_completed == "success")
    {
...
{
    var action_completed = submitFormDynamically('delete_form'+_record_id, 'devaitionractification/'+_record_id, 'Successfully Deleted', 'Please Try Again', 'POST');
    if(action_completed == "success")
    {
        // cancelUpdateForm('add_form', 'update_form');
        fetchLatestRecordsPagination();
...
function fetchLatestRecordsPagination(_category_appendable = 'records_table', _page_number = 0, _appened = 0, _per_page = 10)
{
    ...
    fetchRecordsDynamically(_category_appendable, 'devaitionractificationlistusingpagination', _per_page, _page_number, _appened);
}
